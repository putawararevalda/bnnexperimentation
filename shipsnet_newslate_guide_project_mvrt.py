...
class BayesShipsCNNSmartpool(PyroModule):
    ...
    def _make_prior(self, shape):
        if self.prior_dist == 'gaussian':
            ...
            base = dist.Uniform(-self.prior_b, self.prior_b) #TODO: check if this is correct
        else:
...
from pyro.infer.autoguide import AutoNormal, AutoLowRankMultivariateNormal, AutoGuideList
...
if __name__ == "__main__":
    ...
    parser.add_argument('--var-family', type=str, nargs='?', action='store', default='mean_field',
                help='Variational Family for the SVI Guide. Options are \'mean_field\', \'full_rank_multivariate\'. Default: \'mean_field\'.')
    ...
    for activation_iter in act_fn_list:
        for prior_iter in prior_list:
            for b_iter in b_list:
                ...
                elif args.var_family == 'full_rank_multivariate':
                    if activation_iter == 'gaussian':
                        ...
                        raise ValueError(f"Unsupported variational family and prior combination: {args.var_family} with {activation_iter}")
                    elif activation_iter == 'uniform':
                        raise ValueError(f"Unsupported variational family and prior combination: {args.var_family} with {activation_iter}")
                    else:
                        raise ValueError(f"Unsupported activation: {activation_iter}")
                else:
                    raise ValueError(f"Unsupported variational family: {args.var_family}")
