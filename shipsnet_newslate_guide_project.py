...
def train_svi_with_stats(
    ...
    for epoch in range(1, num_epochs+1):
        ...
        # try to accomodate the inf -inf loss from uniform

        #for images, labels in tqdm(train_loader, desc=f"Epoch {epoch}/{num_epochs}"):
        #    images, labels = images.to(device), labels.to(device).long()
        #    if torch.isinf(loss) or torch.isnan(loss):
        #        print("⚠️ Skipping batch due to bad loss")
        #        continue
        ...
        return_inf_value = None  # Default value for bad loss

        for images, labels in tqdm(train_loader, desc=f"Epoch {epoch}/{num_epochs}"):
            images, labels = images.to(device), labels.to(device).long()

            # Check loss before applying optimizer
            loss = svi.evaluate_loss(images, labels)
            loss_tensor = torch.tensor(loss)

            if torch.isinf(loss_tensor) or torch.isnan(loss_tensor):
                #print("⚠️ Skipping batch due to bad loss")
                if return_inf_value is None:
                    return_inf_value = float('inf') if loss >0 else float('-inf')
                continue

            # Now do training step (safe to proceed)
            loss = svi.step(images, labels)
            total_loss += loss
            batches += 1

        #avg_loss = total_loss / batches if batches > 0 else return_inf_value
        if batches > 0:
            avg_loss = total_loss / batches
        else:
            avg_loss = return_inf_value if return_inf_value is not None else float('nan')
        epoch_losses.append(avg_loss)
        ...
        for prior_iter in prior_list:
            for b_iter in b_list:
                ...
                if activation_iter == 'gaussian':
                    guide = AutoNormal(bayesian_model, init_scale=0.25*b_iter).to(device)
                elif activation_iter == 'laplace':
                    guide = AutoLaplace(bayesian_model, init_scale=0.25*b_iter).to(device)
                elif activation_iter == 'uniform':
                    guide = AutoUniform(bayesian_model, init_scale=0.25*b_iter).to(device)
                else:
