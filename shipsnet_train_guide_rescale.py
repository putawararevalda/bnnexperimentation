...
def plot_training_results_with_stats(losses, accuracies, accuracy_epochs, weight_stats, bias_stats, act_name, prior_name, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'results_GP_shipsnet_newslate_guide_rescale/bayesian_cnn_training_results_{act_name}_{prior_name}_{timestamp}.png')
    #plt.show()
        ...
        for prior_iter in prior_list:
            for b_iter in b_list:
                ...
                if activation_iter == 'gaussian':
                    guide = AutoNormal(bayesian_model, init_scale=10.0).to(device)
                elif activation_iter == 'laplace':
                    guide = AutoLaplace(bayesian_model, init_scale=10.0).to(device)
                elif activation_iter == 'uniform':
                    guide = AutoUniform(bayesian_model, init_scale=10.0).to(device)
                else:
                ...
                num_epochs=10,
                save_epochs=None,
                save_dir='results_GP_shipsnet_newslate_guide_rescale')
                ...
                save_predictions_to_csv(all_labels, all_predictions, os.path.join('results_GP_shipsnet_newslate_guide_rescale', f'predictions_{act_name}_{prior_name}_{experiment_timestamp}_{accuracy * 100:.0f}.csv'))
