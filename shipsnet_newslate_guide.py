...
def plot_training_results_with_stats(losses, accuracies, accuracy_epochs, weight_stats, bias_stats, act_name, prior_name, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'results_GP_shipsnet_newslate_guide/bayesian_cnn_training_results_{act_name}_{prior_name}_{timestamp}.png')
    #plt.show()
    ...
    for activation_iter in act_fn_list:
        for prior_iter in prior_list:
            for b_iter in b_list:
                ...
                num_epochs=100,
                save_epochs=None,
                save_dir='results_GP_shipsnet_newslate_guide')
                ...
                save_predictions_to_csv(all_labels, all_predictions, os.path.join('results_GP_shipsnet_newslate_guide', f'predictions_{act_name}_{prior_name}_{experiment_timestamp}_{accuracy * 100:.0f}.csv'))
